...
int servPos[] = {
  130 //servPos[0](all fingers)
  , 0 //servPos[1](wrist Rotation sensor)
  , 130 //servPos[2](index sensor)
  , 130 //servPos[3](3Fingers)
  , 130 //servPos[4](thumb sensor)
};
...
void setup() {
  ...
  //  pulselen = map (servPos[0], 0, 130, SERVOMIN, SERVOMAX);
...
void RGB(int redval, int greenval, int blueval, int num)
{
  if (num == 1) {
...
void wristRotation() {
  ...
  if (servPos[1] == 130)
    ...
    servPos[1] += 90;
  pulselenW = map (servPos[1], 0, 130, SERVOMIN, SERVOMAX);
...
void Pinch() {
  ...
  if (servPos[1] == 130)
    ...
    servPos[1] = 130;
  IndexServo.write(servPos[1]);
  Thumbservo.write(servPos[1]);

  //  pulselenP = map (servPos[1], 0, 130, SERVOMIN, SERVOMAX);
...
void indexMov() {
  ...
  int pulselenI;
  if (servPos[2] >= 130) {
    for (servPos[2] = 130; servPos[2] >= 0; --servPos[2]) {
      pulselenI = map (servPos[2], 0, 130, SERVOMIN, SERVOMAX);
      pwm.setPWM(IndexFServo, 0, pulselenI);              // tell servo to go to position in variable 'pos'
      delay(15);                       // waits 15ms for the servo to reach the position
    }
  }
  else {
    for (servPos[2] = 0; servPos[2] <= 130; ++servPos[2]) {
      pulselenI = map (servPos[2], 0, 130, SERVOMIN, SERVOMAX);
      pwm.setPWM(IndexFServo, 0, pulselenI);             // tell servo to go to position in variable 'pos'
      delay(15);                       // waits 15ms for the servo to reach the position
    }
...
void fingers_3F() {
  ...
  if (servPos[3] >= 160) {
    for (servPos[3] = 160; servPos[3] >= 0; --servPos[3]) {
      pulselen3F = map (servPos[3], 0, 130, SERVOMIN, SERVOMAX);
...
void ThumbMov() {
  ...
  int pulselenT;
  if (servPos[4] >= 130) {
    for (servPos[4] = 130; servPos[4] >= 0; --servPos[4]) {
      pulselenT = map (servPos[4], 0, 130, SERVOMIN, SERVOMAX);
      pwm.setPWM(ThumbServo, 0, pulselenT);              // tell servo to go to position in variable 'pos'
      delay(15);                       // waits 15ms for the servo to reach the position
    }
  }
  else {
    for (servPos[4] = 0; servPos[4] <= 130; ++servPos[4]) {
      pulselenT = map (servPos[4], 0, 130, SERVOMIN, SERVOMAX);
      pwm.setPWM(ThumbServo, 0, pulselenT);              // tell servo to go to position in variable 'pos'
      delay(15);                       // waits 15ms for the servo to reach the position
    }
